...
const getTwit = async (userId: string) => {
	try {
		const userObjectId: mongoose.Types.ObjectId = userMocking[parseInt(userId) - 1]
		const twitList = await twit.find({
			writer: userObjectId
		}).populate('writer', 'userName').select('-__v -retwitUser')
		const newTwitList = await Promise.all(twitList.map(async (value: any) => {
		...
		const retwitDataList = await Promise.all(retwitList.map(async (value: any) => {
			const twitt: any = await twit.findOne({ _id: value.postId }).select('-__v -retwitUser').populate('writer', 'userName')
